...
def extract_from_gcs(color: str, year: int, month: int) -> Path:
    ...
    gcs_path = f"{color}_tripdata_{year}-{month:02}.csv.gz"
    gcs_block = GcsBucket.load("zoom-gcs")
    gcs_block.get_directory(from_path=gcs_path, local_path=f"../data/fhv/")
    return Path(f"../data/fhv/{gcs_path}")
...
def write_bq(df: pd.DataFrame) -> None:
    ...
    df.to_gbq(
        destination_table="dezoomcamp.fhv_tripdata_materialized",
